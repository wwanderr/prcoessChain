# 进程链生成系统 - 核心算法实现详解

> **文档版本**: v2.0  
> **最后更新**: 2025-12-08  
> **前置阅读**: [01-核心架构与数据流程](./01-核心架构与数据流程.md)

---

## 📋 目录

1. [全树遍历算法](#1-全树遍历算法)
2. [虚拟父节点批量创建算法](#2-虚拟父节点批量创建算法)
3. [告警选举算法](#3-告警选举算法)
4. [智能裁剪算法（图裁剪）](#4-智能裁剪算法图裁剪)
5. [强制裁剪算法（兜底机制）](#5-强制裁剪算法兜底机制)
6. [实体提取算法](#6-实体提取算法)
7. [扩展溯源算法](#7-扩展溯源算法)
8. [角色修正算法](#8-角色修正算法)

---

## 1. 全树遍历算法

### 1.1 算法概述

**算法名称**: `fullTreeTraversal`

**所在类**: `ProcessChainGraph.java`

**功能**: 从任意起点节点出发，收集整个连通子树的所有节点

**适用场景**:
- 子图提取：从告警节点提取相关的进程链
- 支持断链：即使没有根节点也能工作
- 包含所有分支：兄弟节点、侄节点等

### 1.2 算法原理

**核心思想**: 两阶段遍历

```
阶段1: 向上遍历（Traverse Upward）
  从起点向上追溯到根节点（或最顶端节点）
  记录所有访问过的节点
  记录所有到达的顶端节点
  
阶段2: 向下遍历（Traverse Downward）
  从所有顶端节点出发
  向下遍历到所有叶节点
  收集整个连通子图
```

**为什么需要两阶段?**

```
单纯向上遍历的问题:
         ROOT
          |
        NODE_A
       /     \
    NODE_B   NODE_C
      |        |
  NODE_D    NODE_E ⚠️ (起点)
      |
  NODE_F

如果只向上遍历:
  收集: {NODE_E, NODE_C, NODE_A, ROOT}
  遗漏: {NODE_B, NODE_D, NODE_F}  ❌

两阶段遍历:
  阶段1: NODE_E → NODE_C → NODE_A → ROOT
  阶段2: 从ROOT向下遍历整棵树
  收集: {ROOT, NODE_A, NODE_B, NODE_C, NODE_D, NODE_E, NODE_F}  ✅
```

### 1.3 算法实现

#### 核心方法

```java
public Set<String> fullTreeTraversal(String startNodeId) {
    Set<String> visited = new HashSet<>();
    Set<String> topmostNodes = new HashSet<>();
    
    // 阶段1: 向上遍历到根
    traverseUpward(startNodeId, visited, topmostNodes);
    
    // 阶段2: 从所有顶端节点向下遍历
    for (String topmostNode : topmostNodes) {
        traverseDownward(topmostNode, visited);
    }
    
    return visited;
}
```

#### 阶段1: 向上遍历

```java
private void traverseUpward(
        String nodeId, 
        Set<String> visited, 
        Set<String> topmostNodes) {
    
    // 防环：已访问过的节点不再处理
    if (!visited.add(nodeId)) {
        return;
    }
    
    GraphNode node = nodes.get(nodeId);
    if (node == null) {
        return;  // 节点不存在
    }
    
    // 获取所有父节点（使用反向边）
    Set<String> parents = reverseEdges.get(nodeId);
    
    if (parents == null || parents.isEmpty()) {
        // 没有父节点，这是顶端节点
        topmostNodes.add(nodeId);
    } else {
        // 递归向上遍历所有父节点
        for (String parentId : parents) {
            traverseUpward(parentId, visited, topmostNodes);
        }
    }
}
```

**关键点**:
1. 使用 `visited` 集合防止环
2. 收集所有顶端节点（没有父节点的节点）
3. 支持多个父节点的情况（虽然进程链通常是树结构）

#### 阶段2: 向下遍历

```java
private void traverseDownward(
        String nodeId, 
        Set<String> visited) {
    
    // 防环：已访问过的节点不再处理
    if (!visited.add(nodeId)) {
        return;
    }
    
    // 获取所有子节点（使用前向边）
    Set<String> children = edges.get(nodeId);
    
    if (children != null) {
        // 递归向下遍历所有子节点
        for (String childId : children) {
            traverseDownward(childId, visited);
        }
    }
}
```

**关键点**:
1. DFS深度优先遍历
2. 使用 `visited` 集合防止重复访问
3. 遍历所有子分支

### 1.4 复杂度分析

**时间复杂度**: O(V + E)
- V: 节点数
- E: 边数
- 每个节点和边最多访问一次

**空间复杂度**: O(V)
- visited 集合: O(V)
- 递归栈深度: O(h)，h为树的高度

### 1.5 示例演示

**场景1: 有根节点的完整树**

```
输入图:
         ROOT
          |
        NODE_A
       /     \
    NODE_B   NODE_C (起点)
      |        |
  NODE_D    NODE_E
      |
  NODE_F

执行过程:
  阶段1 (向上):
    NODE_C → visited: {NODE_C}
    NODE_A → visited: {NODE_C, NODE_A}
    ROOT → visited: {NODE_C, NODE_A, ROOT}
    topmostNodes: {ROOT}
  
  阶段2 (向下):
    从ROOT开始:
      ROOT → NODE_A → visited: {NODE_C, NODE_A, ROOT}  (已访问,跳过)
      NODE_A → NODE_B → visited: {NODE_C, NODE_A, ROOT, NODE_B}
      NODE_B → NODE_D → visited: {..., NODE_D}
      NODE_D → NODE_F → visited: {..., NODE_F}
      NODE_A → NODE_C → visited: {...}  (已访问,跳过)
      NODE_C → NODE_E → visited: {..., NODE_E}
  
结果: {ROOT, NODE_A, NODE_B, NODE_C, NODE_D, NODE_E, NODE_F}  ✅
```

**场景2: 断链（无根节点）**

```
输入图:
      NODE_A (最顶端，但不是根)
          |
        NODE_B
       /     \
    NODE_C   NODE_D (起点)
      |        |
  NODE_E    NODE_F

执行过程:
  阶段1 (向上):
    NODE_D → NODE_B → NODE_A
    topmostNodes: {NODE_A}
  
  阶段2 (向下):
    从NODE_A开始遍历整棵树
  
结果: {NODE_A, NODE_B, NODE_C, NODE_D, NODE_E, NODE_F}  ✅
```

**场景3: 多个起点**

```
输入图:
         ROOT
          |
        NODE_A
       /     \
    NODE_B (起点1)   NODE_C (起点2)
      |                |
  NODE_D            NODE_E

处理方式:
  分别调用 fullTreeTraversal(NODE_B) 和 fullTreeTraversal(NODE_C)
  然后合并结果（使用Set自动去重）

结果: {ROOT, NODE_A, NODE_B, NODE_C, NODE_D, NODE_E}  ✅
```

---

## 2. 虚拟父节点批量创建算法

### 2.1 算法概述

**算法名称**: `createVirtualParentsForSubgraph`

**所在类**: `ProcessChainGraph.java`

**功能**: 为子图中缺失父节点的节点批量创建虚拟父节点

**核心优化**: 
- 多个子节点共享一个虚拟父节点
- 从子节点的日志/告警中提取父进程信息
- 避免重复创建

### 2.2 算法原理

**问题场景**:

```
原始完整图:
  PARENT_X → NODE_A → NODE_B
             (告警)

子图提取后:
  [PARENT_X缺失] → NODE_A → NODE_B
  
问题: NODE_A的parentProcessGuid指向PARENT_X，但PARENT_X不在子图中
```

**解决方案**:

```
创建虚拟父节点:
  VIRTUAL_PARENT_X ⭐ (虚拟) → NODE_A → NODE_B

虚拟父节点特征:
  - isVirtual = true
  - 从NODE_A的日志/告警中提取父进程信息
  - 只包含基本信息（进程名、路径等）
```

**共享优化**:

```
场景: 多个子节点的父节点相同
  NODE_A (parentGuid = PARENT_SHARED)
  NODE_B (parentGuid = PARENT_SHARED)
  NODE_C (parentGuid = PARENT_SHARED)

传统方案:
  创建3个虚拟父节点  ❌ (冗余)

优化方案:
  只创建1个虚拟父节点  ✅
  
结果:
  PARENT_SHARED ⭐ (虚拟)
     ├─→ NODE_A
     ├─→ NODE_B
     └─→ NODE_C
```

### 2.3 算法实现

#### 核心方法

```java
public void createVirtualParentsForSubgraph(
        Set<String> subgraphNodeIds,
        Set<String> traceIds) {
    
    // 第1步: 收集缺失父节点的节点
    Map<String, List<GraphNode>> missingParents = new HashMap<>();
    
    for (String nodeId : subgraphNodeIds) {
        GraphNode node = nodes.get(nodeId);
        String parentGuid = node.getParentProcessGuid();
        
        // 条件: 有parentGuid && 父节点不在子图中 && 不是根节点
        if (parentGuid != null 
                && !subgraphNodeIds.contains(parentGuid)
                && !traceIds.contains(node.getProcessGuid())) {
            
            missingParents.computeIfAbsent(parentGuid, k -> new ArrayList<>())
                         .add(node);
        }
    }
    
    // 第2步: 批量创建虚拟父节点
    for (Map.Entry<String, List<GraphNode>> entry : missingParents.entrySet()) {
        String parentGuid = entry.getKey();
        List<GraphNode> children = entry.getValue();
        
        // 创建虚拟父节点
        GraphNode virtualParent = createVirtualParentNode(parentGuid, children);
        virtualParent.setIsVirtual(true);
        
        // 添加到图中
        nodes.put(parentGuid, virtualParent);
        
        // 建立父子边
        for (GraphNode child : children) {
            addEdge(parentGuid, child.getProcessGuid());
        }
    }
}
```

#### 创建虚拟父节点

```java
private GraphNode createVirtualParentNode(
        String parentGuid,
        List<GraphNode> children) {
    
    GraphNode virtualParent = new GraphNode();
    virtualParent.setProcessGuid(parentGuid);
    virtualParent.setNodeType("process");
    virtualParent.setIsVirtual(true);
    
    // 尝试从子节点的日志中提取父进程信息
    for (GraphNode child : children) {
        // 优先从日志提取
        if (child.getLogs() != null) {
            for (RawLog log : child.getLogs()) {
                if (parentGuid.equals(log.getParentProcessGuid())) {
                    // 找到了父进程信息
                    extractParentInfoFromLog(virtualParent, log);
                    return virtualParent;
                }
            }
        }
        
        // 其次从告警提取
        if (child.getAlarms() != null) {
            for (RawAlarm alarm : child.getAlarms()) {
                if (parentGuid.equals(alarm.getParentProcessGuid())) {
                    extractParentInfoFromAlarm(virtualParent, alarm);
                    return virtualParent;
                }
            }
        }
    }
    
    // 如果都提取不到，使用默认信息
    virtualParent.addLog(createDefaultLog(parentGuid));
    
    return virtualParent;
}
```

#### 提取父进程信息

```java
private void extractParentInfoFromLog(GraphNode virtualParent, RawLog log) {
    RawLog parentLog = new RawLog();
    parentLog.setProcessGuid(virtualParent.getProcessGuid());
    parentLog.setLogType("process");
    
    // 从子节点日志的父进程字段提取信息
    parentLog.setProcessName(log.getParentProcessName());
    parentLog.setImage(log.getParentImage());
    parentLog.setCommandLine(log.getParentCommandLine());
    
    virtualParent.addLog(parentLog);
}
```

### 2.4 示例演示

**场景1: 单个子节点缺失父节点**

```
原始图:
  PARENT_X (info: cmd.exe) → NODE_A → NODE_B
                              (告警)

子图提取:
  subgraphNodeIds = {NODE_A, NODE_B}

执行过程:
  1. 收集缺失父节点:
     NODE_A.parentGuid = "PARENT_X"
     PARENT_X 不在子图中
     missingParents = {"PARENT_X": [NODE_A]}
  
  2. 创建虚拟父节点:
     从NODE_A的日志中提取父进程信息
     virtualParent = {
       processGuid: "PARENT_X",
       processName: "cmd.exe",  (从日志提取)
       isVirtual: true
     }
  
  3. 建立边:
     addEdge("PARENT_X", "NODE_A")

结果:
  VIRTUAL_PARENT_X ⭐ (cmd.exe) → NODE_A → NODE_B
```

**场景2: 多个子节点共享父节点**

```
原始图:
  PARENT_SHARED → NODE_A
  PARENT_SHARED → NODE_B
  PARENT_SHARED → NODE_C

子图提取:
  subgraphNodeIds = {NODE_A, NODE_B, NODE_C}

执行过程:
  1. 收集缺失父节点:
     missingParents = {"PARENT_SHARED": [NODE_A, NODE_B, NODE_C]}
  
  2. 创建虚拟父节点:
     只创建1个 VIRTUAL_PARENT_SHARED ✅
  
  3. 建立边:
     addEdge("PARENT_SHARED", "NODE_A")
     addEdge("PARENT_SHARED", "NODE_B")
     addEdge("PARENT_SHARED", "NODE_C")

结果:
  VIRTUAL_PARENT_SHARED ⭐
     ├─→ NODE_A
     ├─→ NODE_B
     └─→ NODE_C
```

---

## 3. 告警选举算法

### 3.1 算法概述

**算法名称**: `electAlarms`

**所在类**: `AlarmElectionUtil.java`

**功能**: 从多个告警组中选出最严重的组

**选举规则**:
```
1. 网端关联优先 (associatedEventId匹配)
2. 高危告警组优先 (威胁等级=高)
3. 中危告警组优先 (威胁等级=中)
4. 低危告警组优先 (威胁等级=低)
5. 数量多的优先 (同等级比较数量)
6. 时间早的优先 (最后的tie-breaker)
```

### 3.2 算法实现

```java
public static List<RawAlarm> electAlarms(
        List<RawAlarm> allAlarms,
        String associatedEventId) {
    
    if (allAlarms == null || allAlarms.isEmpty()) {
        return Collections.emptyList();
    }
    
    // 步骤1: 网端关联优先
    if (associatedEventId != null && !associatedEventId.trim().isEmpty()) {
        List<RawAlarm> associated = allAlarms.stream()
            .filter(a -> associatedEventId.equals(a.getEventId()))
            .collect(Collectors.toList());
        
        if (!associated.isEmpty()) {
            return associated;  // 直接返回关联告警
        }
    }
    
    // 步骤2: 按 traceId 分组
    Map<String, List<RawAlarm>> groups = allAlarms.stream()
        .collect(Collectors.groupingBy(RawAlarm::getTraceId));
    
    // 步骤3: 计算每组的威胁统计
    Map<String, ThreatStatistics> stats = new HashMap<>();
    for (Map.Entry<String, List<RawAlarm>> entry : groups.entrySet()) {
        stats.put(entry.getKey(), calculateThreatStats(entry.getValue()));
    }
    
    // 步骤4: 选择最严重的组
    String selectedTraceId = stats.entrySet().stream()
        .max((e1, e2) -> compareThreatStats(e1.getValue(), e2.getValue()))
        .map(Map.Entry::getKey)
        .orElse(null);
    
    return groups.get(selectedTraceId);
}
```

### 3.3 威胁统计

```java
class ThreatStatistics {
    int highCount;     // 高危数量
    int mediumCount;   // 中危数量
    int lowCount;      // 低危数量
    int totalCount;    // 总数
}

private static ThreatStatistics calculateThreatStats(List<RawAlarm> alarms) {
    ThreatStatistics stats = new ThreatStatistics();
    
    for (RawAlarm alarm : alarms) {
        String severity = alarm.getThreatSeverity();
        
        if ("高".equals(severity) || "HIGH".equalsIgnoreCase(severity)) {
            stats.highCount++;
        } else if ("中".equals(severity) || "MEDIUM".equalsIgnoreCase(severity)) {
            stats.mediumCount++;
        } else if ("低".equals(severity) || "LOW".equalsIgnoreCase(severity)) {
            stats.lowCount++;
        }
        
        stats.totalCount++;
    }
    
    return stats;
}
```

### 3.4 威胁比较

```java
private static int compareThreatStats(
        ThreatStatistics stats1,
        ThreatStatistics stats2) {
    
    // 1. 比较高危数量
    if (stats1.highCount != stats2.highCount) {
        return stats1.highCount - stats2.highCount;
    }
    
    // 2. 比较中危数量
    if (stats1.mediumCount != stats2.mediumCount) {
        return stats1.mediumCount - stats2.mediumCount;
    }
    
    // 3. 比较低危数量
    if (stats1.lowCount != stats2.lowCount) {
        return stats1.lowCount - stats2.lowCount;
    }
    
    // 4. 比较总数
    return stats1.totalCount - stats2.totalCount;
}
```

### 3.5 示例演示

**场景: 多个告警组选举**

```
输入:
  告警组1 (traceId=T001):
    - 高危×2, 中危×1  → stats: {high:2, medium:1, low:0, total:3}
  
  告警组2 (traceId=T002):
    - 高危×1, 中危×3  → stats: {high:1, medium:3, low:0, total:4}
  
  告警组3 (traceId=T003):
    - 中危×5          → stats: {high:0, medium:5, low:0, total:5}

比较过程:
  T001 vs T002:
    高危: 2 > 1  → T001 胜出
  
  T001 vs T003:
    高危: 2 > 0  → T001 胜出

结果: 选择告警组1 (T001) ✅
```

---

## 4. 智能裁剪算法（图裁剪）

### 4.1 算法概述

**算法名称**: `pruneGraph`

**所在类**: `GraphPruner.java`

**触发时机**: 实体提取之前（阶段6）

**触发条件**: 节点数 > `MAX_NODE_COUNT`（默认400）

**目标**: 将进程节点数量控制在合理范围内

**核心策略**: 基于重要性评分的智能裁剪

### 4.2 重要性评分规则

#### 基础分（100分）
所有节点的起始分数

#### 加分项
```java
1. 告警节点：+50分
2. 网端关联节点：+100分（最高优先级）
3. 断链节点：+30分
4. 根节点：+20分
5. 子节点数量：每个子节点 +5分（最多+50分）
```

#### 减分项
```java
1. 深度惩罚：深度每增加1，-10分
2. 叶子节点：-20分
```

### 4.3 算法流程

```
输入: 节点数 = 500（超过阈值400）

步骤1: 计算所有节点的重要性评分
  NODE_A: 100 + 50(告警) + 20(根节点) = 170分
  NODE_B: 100 + 100(网端关联) + 50(告警) = 250分
  NODE_C: 100 - 20(叶子) - 10(深度1) = 70分
  ...

步骤2: 按评分降序排序
  [NODE_B(250), NODE_A(170), ..., NODE_C(70)]

步骤3: 保留前 MAX_NODE_COUNT 个节点
  保留前400个高分节点

步骤4: 移除低分节点
  移除100个低分节点

步骤5: 清理孤立节点和无效边
  移除没有父节点也没有子节点的孤立节点
  移除指向已删除节点的边
```

### 4.4 保护机制

```java
// 永远保留的节点
1. 告警节点（isAlarm = true）
2. 网端关联节点（在 networkAssociatedEventIds 中）
3. 根节点（优先保留）
4. 断链节点（优先保留）
```

### 4.5 核心实现

```java
public static void pruneGraph(
        ProcessChainGraph graph,
        int maxNodeCount,
        Set<String> networkAssociatedEventIds) {
    
    // 步骤1: 计算所有节点的重要性评分
    Map<String, Integer> scores = new HashMap<>();
    for (GraphNode node : graph.getAllNodes()) {
        int score = calculateImportanceScore(node, graph, networkAssociatedEventIds);
        scores.put(node.getNodeId(), score);
    }
    
    // 步骤2: 按评分降序排序
    List<Map.Entry<String, Integer>> sortedNodes = new ArrayList<>(scores.entrySet());
    sortedNodes.sort((a, b) -> b.getValue().compareTo(a.getValue()));
    
    // 步骤3: 保留前 maxNodeCount 个节点
    Set<String> nodesToKeep = new LinkedHashSet<>();
    for (int i = 0; i < Math.min(maxNodeCount, sortedNodes.size()); i++) {
        nodesToKeep.add(sortedNodes.get(i).getKey());
    }
    
    // 步骤4: 执行裁剪
    graph.retainNodes(nodesToKeep);
    
    // 步骤5: 清理孤立节点
    graph.removeOrphanNodes();
}

// 计算重要性评分
private static int calculateImportanceScore(
        GraphNode node,
        ProcessChainGraph graph,
        Set<String> networkAssociatedEventIds) {
    
    int score = 100;  // 基础分
    
    // 加分项
    if (node.isAlarm()) {
        score += 50;  // 告警节点
    }
    
    if (isNetworkAssociated(node, networkAssociatedEventIds)) {
        score += 100;  // 网端关联节点（最高优先级）
    }
    
    if (node.isBroken()) {
        score += 30;  // 断链节点
    }
    
    if (graph.isRootNode(node.getNodeId())) {
        score += 20;  // 根节点
    }
    
    // 子节点数量加分
    int childCount = graph.getChildren(node.getNodeId()).size();
    score += Math.min(childCount * 5, 50);  // 最多+50分
    
    // 减分项
    int depth = graph.getNodeDepth(node.getNodeId());
    score -= depth * 10;  // 深度惩罚
    
    if (childCount == 0) {
        score -= 20;  // 叶子节点
    }
    
    return score;
}
```

### 4.6 裁剪效果示例

```
裁剪前:
  节点数: 500
  告警节点: 10
  网端关联节点: 5
  普通节点: 485

裁剪后:
  节点数: 400
  告警节点: 10 ✅ (全部保留)
  网端关联节点: 5 ✅ (全部保留)
  高分节点: 385
  移除节点: 100 (低分节点)
```

---

## 5. 强制裁剪算法（兜底机制）

### 5.1 算法概述

**算法名称**: `forcePrune`

**所在类**: `ForcePruner.java`

**触发时机**: 实体过滤之后（阶段7.5）

**触发条件**: 节点数 > 100

**目标**: 裁剪到 `MAX_NODE_COUNT`（默认20）

**核心策略**:
1. 按 traceId 分组（最多3个traceId）
2. 平分配额
3. 优先保留网端关联节点和关键路径
4. **单链模式**：避免树杈，只保留一条完整的链
5. 确定性算法（结果可复现）

**为什么需要强制裁剪？**
- 智能裁剪后，实体提取可能导致节点数爆炸
- 某些场景下，一个进程节点可能有上百个实体子节点
- 需要兜底机制确保最终节点数在可控范围内

### 4.2 算法流程

```
输入: 节点数 = 150（超过阈值100）

步骤1: 按 traceId 分组
  traceId_1: 60个节点
  traceId_2: 50个节点
  traceId_3: 40个节点

步骤2: 选择 top 3 traceId
  (如果超过3个，按字典序选择前3个)

步骤3: 计算配额
  总配额: 30
  每个traceId: 30 / 3 = 10个节点

步骤4: 为每个traceId选择节点
  优先级:
    1. 网端关联进程节点
    2. 关联节点到根节点的单链
    3. 网端关联实体节点
    4. 其他节点（深度优先 + GUID字典序）

步骤5: 执行裁剪
  保留选中的30个节点
  移除其他120个节点
```

### 4.3 核心实现

```java
public static ForcePruneResult forcePrune(
        ProcessChainGraph graph,
        Set<String> networkAssociatedEventIds,
        Set<String> traceIds) {
    
    // 步骤1: 按 traceId 分组
    Map<String, TraceGroup> traceGroups = groupByTraceId(graph, traceIds);
    
    // 步骤2: 选择 top 3 traceId
    List<String> selectedTraceIds = selectTopTraceIds(traceGroups, 3);
    
    // 步骤3: 计算配额
    int quotaPerTrace = FORCE_PRUNE_TARGET / selectedTraceIds.size();
    int remainder = FORCE_PRUNE_TARGET % selectedTraceIds.size();
    
    // 步骤4: 为每个 traceId 选择节点
    Set<String> nodesToKeep = new LinkedHashSet<>();
    
    for (int i = 0; i < selectedTraceIds.size(); i++) {
        String traceId = selectedTraceIds.get(i);
        TraceGroup group = traceGroups.get(traceId);
        
        // 第一个 traceId 分配余数
        int quota = quotaPerTrace + (i == 0 ? remainder : 0);
        
        Set<String> selected = selectNodesForTrace(
            group, quota, graph, networkAssociatedEventIds
        );
        
        nodesToKeep.addAll(selected);
    }
    
    // 步骤5: 执行裁剪
    graph.retainNodes(nodesToKeep);
    
    return new ForcePruneResult(
        true,  // 裁剪成功
        nodesToKeep.size(),
        graph.getNodeCount()
    );
}
```

### 4.4 节点选择策略

```java
private static Set<String> selectNodesForTrace(
        TraceGroup group,
        int quota,
        ProcessChainGraph graph,
        Set<String> networkAssociatedEventIds) {
    
    Set<String> selected = new LinkedHashSet<>();
    
    // 步骤1: 保留网端关联进程节点
    Set<String> associatedProcessNodes = findAssociatedProcessNodes(
        group, networkAssociatedEventIds
    );
    selected.addAll(associatedProcessNodes);
    
    // 步骤2: 级联保留单链（关联节点 → 根节点）
    for (String associatedNode : associatedProcessNodes) {
        Set<String> chainNodes = traceUpwardToRoot(
            associatedNode, group.rootNodeId, graph
        );
        selected.addAll(chainNodes);
    }
    
    // 步骤3: 保留网端关联的实体节点
    Set<String> associatedEntityNodes = findAssociatedEntityNodes(
        group, networkAssociatedEventIds
    );
    selected.addAll(associatedEntityNodes);
    
    // 步骤4: 如果还有配额，保留其他节点
    if (selected.size() < quota) {
        int remaining = quota - selected.size();
        Set<String> otherNodes = selectRemainingNodes(
            group, selected, remaining, graph
        );
        selected.addAll(otherNodes);
    }
    
    // 步骤5: 如果超过配额，裁剪到配额
    if (selected.size() > quota) {
        selected = trimToQuota(selected, quota);
    }
    
    return selected;
}
```

### 4.5 单链模式与无根节点场景修复

#### 4.5.1 什么是单链？

**定义**: 从一个起点开始，向上追溯到根节点（或断链节点），向下选择子节点（只选第一个子节点），全程不分叉

**示例**:
```
问题场景（多分支）:
         ROOT
          |
        NODE_A
       /  |  \
    NODE_B NODE_C NODE_D
      |      |      |
   (100个)  (50个)  (30个)
   
如果保留所有分支:
  节点数 = 1 + 1 + 3 + 180 = 185个  ❌ 超过配额

单链模式:
  只保留: ROOT -> NODE_A -> NODE_B -> 部分实体
  节点数 = 1 + 1 + 1 + 17 = 20个  ✅ 符合配额
```

#### 4.5.2 无根节点场景的问题（修复前）

**场景**: 没有根节点，只有断链节点

```
数据结构:
  - PPP10001 (断链节点) -> AAA10001（4个实体）
  - PPP10002 (断链节点) -> AAA10002（11个实体）
  - PPP10003 (断链节点) -> AAA10003（0个实体）

修复前的逻辑:
  1. 选择第一个进程节点（AAA10001）
  2. 向下DFS（没有子进程，只有1个节点）
  3. 添加所有实体（包括其他链的实体）

结果:
  - AAA10001（1个进程）✅
  - AAA10001的4个实体 ✅
  - AAA10002的11个实体 ❌ (不应该保留)
  - 其他实体 ❌
  - 丢失了PPP10001（断链节点）❌
  
问题:
  ❌ 丢失断链节点，无法桥接网端
  ❌ 包含了其他链的实体，破坏单链
```

#### 4.5.3 修复方案

**核心改动**:
1. **向上追溯**: 从选中节点向上找到断链节点
2. **反转链条**: 让断链节点在前，子节点在后
3. **只添加单链上的实体**: 只遍历已选中进程节点的直接子实体

**修复后的代码**:
```java
/**
 * 按 GUID 顺序选择单链（用于没有根节点的场景）
 */
private static Set<String> selectByGuidOrder(
        TraceGroup group,
        ProcessChainGraph graph,
        Set<String> excludeNodes,
        int quota) {
    
    Set<String> result = new LinkedHashSet<>();
    
    // 1. 找出所有进程节点并按GUID排序
    List<String> processNodes = new ArrayList<>();
    for (GraphNode node : group.getNodes()) {
        if (!excludeNodes.contains(node.getNodeId()) && 
            node.getNodeType() == NodeType.PROCESS) {
            processNodes.add(node.getNodeId());
        }
    }
    Collections.sort(processNodes);
    
    String firstProcessNode = processNodes.get(0);
    
    // ✅ 2. 向上追溯到断链节点/根节点
    List<String> upwardChain = traceToRootProcessOnly(firstProcessNode, graph);
    Collections.reverse(upwardChain);  // 反转，让根节点在前
    
    for (String nodeId : upwardChain) {
        if (result.size() >= quota) break;
        result.add(nodeId);
    }
    
    // 3. 向下DFS选择子进程节点（单链）
    if (result.size() < quota) {
        dfsSelectProcessNodes(firstProcessNode, graph, excludeNodes, result, quota);
    }
    
    // ✅ 4. 只添加单链上进程节点的直接子实体
    if (result.size() < quota) {
        List<String> directChildEntities = new ArrayList<>();
        
        for (String processNodeId : result) {
            GraphNode processNode = graph.getNode(processNodeId);
            if (processNode != null && processNode.getNodeType() == NodeType.PROCESS) {
                for (String childId : graph.getChildren(processNodeId)) {
                    GraphNode childNode = graph.getNode(childId);
                    if (childNode != null && 
                        childNode.getNodeType() != NodeType.PROCESS &&
                        !excludeNodes.contains(childId) &&
                        !result.contains(childId)) {
                        directChildEntities.add(childId);
                    }
                }
            }
        }
        
        Collections.sort(directChildEntities);
        for (String entityId : directChildEntities) {
            if (result.size() >= quota) break;
            result.add(entityId);
        }
    }
    
    return result;
}
```

**修复后的效果**:
```
输入: 25个节点，配额20

执行过程:
  1. 选择第一个进程节点: AAA10001
  2. 向上追溯: [AAA10001, PPP10001]
  3. 反转: [PPP10001, AAA10001]
  4. 向下DFS: 无子进程
  5. 添加单链上的实体: AAA10001的4个实体

结果:
  - PPP10001（断链节点）✅
  - AAA10001（告警节点）✅
  - AAA10001_REGISTRY_50b14c02 ✅
  - AAA10001_REGISTRY_9e148fd2 ✅
  - AAA10001_REGISTRY_5eca3b05 ✅
  - AAA10001_NETWORK_7b2d620a ✅
  总计: 6个节点

效果:
  ✅ 保留了断链节点，可以创建虚拟根节点
  ✅ 形成完整单链，没有分叉
  ✅ 可以正确桥接网端和端侧
  ✅ 不包含AAA10002、AAA10003及其实体
```

---

## 6. 实体提取算法

### 6.1 算法概述

**算法名称**: `extractEntities`

**所在类**: `EntityExtractor.java`

**功能**: 从裁剪后的进程节点提取实体（文件、域名、网络、注册表）

**核心优化**: 延迟拆分 - 只为保留的进程提取实体

### 5.2 算法流程

```
输入: 裁剪后的30个进程节点

步骤1: 遍历进程节点
  for each processNode:
    按 logType 分组日志

步骤2: 提取各类实体
  - file 日志 → FileEntity
  - domain 日志 → DomainEntity
  - network 日志 → NetworkEntity
  - registry 日志 → RegistryEntity

步骤3: 创建实体节点
  - 生成唯一ID
  - 设置 nodeType = "xxx_entity"
  - 设置 parentProcessGuid = processNode.id

步骤4: 建立关联边
  - 创建边: processNode → entityNode

输出: 实体节点 + 关联边
```

### 5.3 核心实现

```java
public static EntityExtractionResult extractEntities(
        Map<String, GraphNode> processNodes,
        Map<String, GraphNode> allNodes) {
    
    Map<String, GraphNode> entityNodes = new HashMap<>();
    List<Edge> entityEdges = new ArrayList<>();
    
    for (GraphNode processNode : processNodes.values()) {
        List<RawLog> logs = processNode.getLogs();
        
        if (logs == null || logs.isEmpty()) {
            // 无日志，尝试从告警提取
            extractFromAlarms(processNode, entityNodes, entityEdges);
            continue;
        }
        
        // 按 logType 分组
        Map<String, List<RawLog>> logsByType = logs.stream()
            .collect(Collectors.groupingBy(RawLog::getLogType));
        
        // 提取文件实体
        extractFileEntities(
            logsByType.get("file"), 
            processNode, 
            entityNodes, 
            entityEdges
        );
        
        // 提取域名实体
        extractDomainEntities(
            logsByType.get("domain"), 
            processNode, 
            entityNodes, 
            entityEdges
        );
        
        // 提取网络实体
        extractNetworkEntities(
            logsByType.get("network"), 
            processNode, 
            entityNodes, 
            entityEdges
        );
        
        // 提取注册表实体
        extractRegistryEntities(
            logsByType.get("registry"), 
            processNode, 
            entityNodes, 
            entityEdges
        );
    }
    
    return new EntityExtractionResult(entityNodes, entityEdges);
}
```

### 5.4 文件实体提取

```java
private static void extractFileEntities(
        List<RawLog> fileLogs,
        GraphNode processNode,
        Map<String, GraphNode> entityNodes,
        List<Edge> entityEdges) {
    
    if (fileLogs == null || fileLogs.isEmpty()) {
        return;
    }
    
    for (RawLog fileLog : fileLogs) {
        // 生成实体ID（使用MD5避免重复）
        String entityId = generateEntityId(fileLog);
        
        // 避免重复创建
        if (entityNodes.containsKey(entityId)) {
            continue;
        }
        
        // 创建实体节点
        GraphNode entityNode = new GraphNode();
        entityNode.setProcessGuid(entityId);
        entityNode.setNodeType("file_entity");
        entityNode.setParentProcessGuid(processNode.getProcessGuid());
        entityNode.addLog(fileLog);
        
        entityNodes.put(entityId, entityNode);
        
        // 创建边: 进程 → 文件
        Edge edge = new Edge(
            processNode.getProcessGuid(),
            entityId,
            "文件操作"
        );
        entityEdges.add(edge);
    }
}
```

### 5.5 实体ID生成

```java
private static String generateEntityId(RawLog log) {
    StringBuilder sb = new StringBuilder();
    
    // 文件实体: processGuid + targetFilename + eventType
    if ("file".equals(log.getLogType())) {
        sb.append(log.getProcessGuid())
          .append("|")
          .append(log.getTargetFilename())
          .append("|")
          .append(log.getEventType());
    }
    // 域名实体: processGuid + queryName
    else if ("domain".equals(log.getLogType())) {
        sb.append(log.getProcessGuid())
          .append("|")
          .append(log.getQueryName());
    }
    // 网络实体: processGuid + destIp + destPort
    else if ("network".equals(log.getLogType())) {
        sb.append(log.getProcessGuid())
          .append("|")
          .append(log.getDestinationIp())
          .append("|")
          .append(log.getDestinationPort());
    }
    
    // 使用MD5生成唯一ID
    return "ENTITY_" + md5(sb.toString());
}
```

---

## 7. 扩展溯源算法

### 6.1 算法概述

**算法名称**: `performExtension`

**所在类**: `ProcessChainExtensionUtil.java`

**功能**: 从根节点向上追溯父/祖父进程（最多2层）

**使用场景**: 查看根节点的上下文，了解更完整的攻击源头

### 6.2 算法流程

```
输入: 根节点列表

步骤1: 遍历根节点
  跳过: EXPLORE节点、虚拟根父节点、断链节点

步骤2: 向上扩展
  查询父进程日志
  创建扩展节点（extensionDepth=1）
  继续查询祖父进程日志
  创建扩展节点（extensionDepth=2）

步骤3: 更新isRoot标记
  原根节点: isRoot = false
  最顶端节点: isRoot = true

步骤4: 调整桥接点（如果有）
  原桥接: 网侧 → ROOT
  新桥接: 网侧 → GRANDPARENT

输出: 扩展后的节点和边
```

### 6.3 核心实现

```java
public static Map<String, String> performExtension(
        Map<String, String> traceIdToRootMap,
        Map<String, String> hostToTraceId,
        List<ProcessNode> allNodes,
        List<ProcessEdge> allEdges,
        OptimizedESQueryService esQueryService,
        int maxDepth) {
    
    Map<String, String> updatedMap = new HashMap<>();
    
    for (Map.Entry<String, String> entry : traceIdToRootMap.entrySet()) {
        String traceId = entry.getKey();
        String rootNodeId = entry.getValue();
        
        // 跳过不需要扩展的节点
        if (shouldSkip(rootNodeId, allNodes)) {
            updatedMap.put(traceId, rootNodeId);
            continue;
        }
        
        // 向上扩展
        String topmostNodeId = extendFromNode(
            rootNodeId,
            hostToTraceId.get(traceId),
            allNodes,
            allEdges,
            esQueryService,
            maxDepth
        );
        
        updatedMap.put(traceId, topmostNodeId);
    }
    
    return updatedMap;
}
```

### 6.4 向上扩展

```java
private static String extendFromNode(
        String startNodeId,
        String hostAddress,
        List<ProcessNode> allNodes,
        List<ProcessEdge> allEdges,
        OptimizedESQueryService esQueryService,
        int maxDepth) {
    
    ProcessNode currentNode = findNodeById(startNodeId, allNodes);
    String currentNodeId = startNodeId;
    int depth = 0;
    
    while (depth < maxDepth) {
        // 查询父进程日志
        String parentGuid = queryParentGuid(currentNode);
        if (parentGuid == null) {
            break;  // 无法继续扩展
        }
        
        RawLog parentLog = esQueryService.queryLogByGuid(
            parentGuid, 
            hostAddress
        );
        
        if (parentLog == null) {
            break;  // 查询不到父进程
        }
        
        // 创建扩展节点
        ProcessNode parentNode = createExtensionNode(
            parentLog,
            depth + 1,
            currentNodeId
        );
        
        allNodes.add(parentNode);
        
        // 创建边
        ProcessEdge edge = new ProcessEdge();
        edge.setSource(parentNode.getNodeId());
        edge.setTarget(currentNodeId);
        edge.setVal("进程创建");
        
        allEdges.add(edge);
        
        currentNodeId = parentNode.getNodeId();
        currentNode = parentNode;
        depth++;
    }
    
    // 更新isRoot标记
    if (depth > 0) {
        findNodeById(startNodeId, allNodes)
            .getChainNode()
            .setIsRoot(false);
        
        findNodeById(currentNodeId, allNodes)
            .getChainNode()
            .setIsRoot(true);
    }
    
    return currentNodeId;
}
```

---

## 8. 角色修正算法

### 7.1 算法概述

**算法名称**: `correctRoles`

**所在类**: `NetworkNodeRoleCorrector.java`

**功能**: 修正网络节点的攻击者/受害者角色

**问题**: 有些进程链是反向的（从受害者到攻击者）

### 7.2 识别反向链

```
正向链（正常）:
  攻击者IP → 跳板机 → 受害者IP (目标主机)
  
反向链（需要修正）:
  受害者IP (目标主机) → 跳板机 → 攻击者IP
```

**识别规则**:

```java
boolean isReverse = 
    firstNode.getHostAddress().equals(targetHost) &&
    !lastNode.getHostAddress().equals(targetHost);
```

### 7.3 修正策略

```java
public static boolean correctRoles(
        List<ProcessNode> networkNodes,
        String targetHost) {
    
    if (networkNodes == null || networkNodes.isEmpty()) {
        return false;
    }
    
    // 判断是否是反向链
    ProcessNode firstNode = networkNodes.get(0);
    ProcessNode lastNode = networkNodes.get(networkNodes.size() - 1);
    
    boolean isReverse = 
        firstNode.getHostAddress().equals(targetHost) &&
        !lastNode.getHostAddress().equals(targetHost);
    
    if (!isReverse) {
        return false;  // 不需要修正
    }
    
    // 修正角色
    for (ProcessNode node : networkNodes) {
        NodeColor color = node.getNodeColor();
        
        if (color != null) {
            // 反转角色
            if ("victim".equals(color.getRole())) {
                color.setRole("attacker");
                color.setColor("#FF6B6B");  // 红色
            } else if ("attacker".equals(color.getRole())) {
                color.setRole("victim");
                color.setColor("#4ECDC4");  // 蓝色
            }
        }
    }
    
    return true;
}
```

---

**最后更新**: 2025-12-08  
**文档维护者**: 开发团队

